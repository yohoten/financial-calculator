...
APP_TITLE = "货币时间价值与资本成本计算器"
APP_VERSION = "v2.0 单文件版"
...
class SelfTest:
    ...
    def run(self) -> Tuple[bool, List[str], List[Dict[str, Any]]]:
        ...
        self.lines += ["", "=" * 78,
                       ...
                       "全部数值断言通过（与教材系数表、解析解逐项比对）。" if self.failed == 0
                       else "存在未通过项，请检查计算引擎。",
                       "=" * 78]
...
# ---- 界面配色 ----
UI = {
    "bg": "#F1F5F9", "panel": "#FFFFFF", "header": "#1D4ED8", "header_text": "#FFFFFF",
    "h1": "#1D4ED8", "h2": "#0F766E", "ok": "#047857", "warn": "#B45309", "err": "#B91C1C",
    "muted": "#475569", "body": "#1E293B",
}
...
class PreviewPanel(ttk.LabelFrame):
    ...
    def __init__(self, master, app, title: str = "图表预览"):
        ...
        self._blank.put("#FFFFFF", to=(0, 0, 640, 430))
        self.canvas_label = tk.Label(self, image=self._blank, bg="white",
    ...
    def show(self, path: str) -> None:
        ...
        self.canvas_label.configure(image=img, text="")
...
class ResultText(ttk.Frame):
    ...
    def __init__(self, master):
        ...
        self.text = ScrolledText(self, wrap="word", font=("Consolas", 10),
                                 bg=UI["panel"], fg=UI["body"], relief="solid", bd=1,
                                 padx=10, pady=8, height=22)
        self.text.pack(fill="both", expand=True)
        self.text.configure(state="disabled")
        self.text.tag_configure("h1", font=("Microsoft YaHei", 13, "bold"), foreground=UI["h1"],
...
class SceneTab(ttk.Frame):
    ...
    def __init__(self, master, app, title: str, desc: str,
        ...
        ttk.Label(self, text=title, font=("Microsoft YaHei", 13, "bold"),
                  foreground=UI["h1"]).pack(anchor="w")
        if desc:
    ...
    def _build_params(self, parent: ttk.Frame) -> None:
        ...
        ttk.Button(btns, text="▶  计算并生成图表", command=self.on_calc).pack(side="left")
        ttk.Button(btns, text="恢复示例值", command=self.on_reset).pack(side="left", padx=6)
...
class OverviewTab(ttk.Frame):
    ...
    def __init__(self, master, app):
        ...
        ttk.Label(self, text="总览 · 批量运行与自检", font=("Microsoft YaHei", 13, "bold"),
                  foreground=UI["h1"]).pack(anchor="w")
        ttk.Label(self, text="一键生成全部 8 张图表与 Excel / Markdown 汇总，或运行数值自检测试"
                             "（与教材系数表、解析解逐项比对）。",
                  foreground=UI["muted"]).pack(anchor="w", pady=(2, 8))

        bar = ttk.Frame(self)
        bar.pack(fill="x", pady=(0, 8))
        ttk.Button(bar, text="▶  一键运行全部场景", command=self.on_demo).pack(side="left")
        ttk.Button(bar, text="🧪  运行数值自检测试", command=self.on_selftest).pack(side="left", padx=6)
        ttk.Button(bar, text="📄  导出测试用例簿", command=self.on_casebook).pack(side="left")
        ttk.Button(bar, text="📂  打开输出目录",
                   command=lambda: _open_path(get_output_dir())).pack(side="left", padx=6)
        ttk.Button(bar, text="ℹ  使用说明", command=self.app.show_help).pack(side="left")
...
class FinanceCalculatorApp:
    ...
    def _setup_style(self) -> None:
        ...
        style.configure("TLabelframe", background=UI["bg"])
        ...
        style.configure("TNotebook", background=UI["bg"])
        style.configure("TNotebook.Tab", font=("Microsoft YaHei", 9, "bold"), padding=(12, 6))
        style.configure("Error.TEntry", fieldbackground="#FEE2E2")

    def _build_header(self) -> None:
        head = tk.Frame(self.root, bg=UI["header"], height=62)
        head.pack(fill="x")
        head.pack_propagate(False)
        tk.Label(head, text=APP_TITLE, bg=UI["header"], fg=UI["header_text"],
                 font=("Microsoft YaHei", 15, "bold")).pack(side="left", padx=16)
        tk.Label(head, text="第 3 章 复利 / 年金    ·    第 4 章 债务 / 股权 / WACC",
                 bg=UI["header"], fg="#DBEAFE", font=("Microsoft YaHei", 9)).pack(side="left")
        tk.Label(head, text=APP_VERSION, bg=UI["header"], fg="#BFDBFE",
                 font=("Microsoft YaHei", 9)).pack(side="right", padx=16)

    def _build_toolbar(self) -> None:
        bar = ttk.Frame(self.root, padding=(10, 8, 10, 4))
        bar.pack(fill="x")
        ttk.Button(bar, text="▶  一键运行全部场景（8 图 + Excel + Markdown）",
                   command=self.run_demo).pack(side="left")
        ttk.Button(bar, text="🧪  数值自检测试", command=self.run_selftest).pack(side="left", padx=6)
        ttk.Button(bar, text="📂  打开输出目录",
                   command=lambda: _open_path(get_output_dir())).pack(side="left")
        ttk.Button(bar, text="ℹ  使用说明", command=self.show_help).pack(side="left", padx=6)
